...
3. **`trip_events` là append-only.** Chuyển trạng thái và ghi sự kiện phải nằm trong **cùng một transaction**
   (`Repository.Save(ctx, trip, event)`). Đây là hợp đồng vận tải điện tử theo Nghị định 10/2020 — lưu ≥ 3 năm.
